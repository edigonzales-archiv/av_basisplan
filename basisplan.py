...
class Settings:
  ...
  def __init__(self):
    ...
    self._base_wms_url = "http://www.catais.org/wms/"
    ...
    self._clip_cutline = "PG:\"host=localhost dbname=xanadu2_test user=mspublic password=mspublic\""
    ...
    ##self._clip_cutlinesql = "\"SELECT ST_SetSRID(ST_Buffer(geometrie,<<<buffer_radius>>>),-1) FROM (SELECT ST_Union(ARRAY(SELECT geometrie FROM av_basisplan.maske_kanton WHERE art='Kantonsfläche' UNION SELECT ST_Buffer(geometrie,2.0 / 3.0 * <<<buffer_radius>>>,'endcap=flat') FROM av_avdpool_ch.landesgrenzen_landesgrenzabschnitt)) AS geometrie) AS kantonsflache_und_landesgrenzbuffer\""
    ## Für Kantone ohne Landesgrenzabschnitt kann folgende vereinfachte Abfrage verwendet werden:
    self._clip_cutlinesql = "\"SELECT ST_SetSRID(ST_Buffer(geometrie,<<<buffer_radius>>>),-1) FROM av_basisplan.maske_kanton WHERE ktnr = 5 AND art='Kantonsfläche'\""
  
  ## WMS URL
  def get_wms_url(self):
    return self._base_wms_url + "bpav" + str(self.scale) + self.colortype
...
def postprocessSituationSw(settings, tileinfo, numrequests):
  ...
  try:
    ...
    ## Clippen ALLES! Test
    ## oben kann das jetzt gelöscht werden
    ## Liegenschafen noch anschauen. Unten
    
    if settings.clip == True:    
      cmd = "gdalwarp -q -wm 512 -t_srs EPSG:21781-overwrite -dstnodata 0 -of GTiff -cutline " + settings.get_clip_cutline() + " -csql " + settings.get_clip_cutlinesql() + " " + os.path.join(settings.outdir, basefilename + ".vrt") + " " + os.path.join(settings.outdir, basefilename + "_clipped.tif")
      os.system(cmd)
    ...
    if settings.clip == True:    
      cmd += os.path.join(settings.outdir, basefilename + "_clipped.tif") + " " + os.path.join(settings.outdir, basefilename + ".tif")
    else:
    ...
    if not settings.simulate and not settings.keep_tempfiles:
      ...
      ## clipped...
      deleteFile(os.path.join(settings.outdir, basefilename + "_clipped.tif"))
