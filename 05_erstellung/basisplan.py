...
class Settings:
  ...
  def __init__(self):
    ...
    self.overlapx = 100.0
    self.overlapy = 50.0
    ## Kantonskürzel (wird für WMS-Url gebraucht / Stefan Ziegler, 2013-05-26)
    self.canton = "sz"
...
#    self._base_wms_url = "http://localhost/cgi-bin/qgis_mapserv.fcgi?map=/usr/lib/cgi-bin/"
    ...
    self._clip_cutline = "PG:\"host=localhost dbname=bpav_sz user=mspublic password=mspublic\""
...
#    self._clip_cutlinesql = "\"SELECT ST_SetSRID(ST_Buffer(geom,<<<buffer_radius>>>),-1) FROM (SELECT ST_Union(ARRAY(SELECT geometrie FROM av_basisplan.maske_kanton WHERE art='kantonsflaeche' UNION SELECT ST_Buffer(geometrie,2.0 / 3.0 * <<<buffer_radius>>>,'endcap=flat') FROM av_avdpool_ch.landesgrenzen_landesgrenzabschnitt)) AS geometrie) AS kantonsflache_und_landesgrenzbuffer\""
    ## Für Kantone ohne Landesgrenzabschnitt kann folgende vereinfachte Abfrage verwendet werden:
    ## Musse man noch generisch machen, so dass es für alle Kantone gleich funktioniert! Stefan Ziegler / 2014-05-26
    self._clip_cutlinesql = "\"SELECT ST_SetSRID(ST_Buffer(geom,<<<buffer_radius>>>),-1) FROM av_basisplan.maske_kanton WHERE art='kantonsflaeche'\""
  
  ## WMS URL
  def get_wms_url(self):
    return self._base_wms_url + "bpav" + str(self.scale) + self.colortype + "_" + self.canton
...
def parse_options():
  ...
  optparser.add_option("--canton",
    action = "store",
    type = "string",
    dest = "canton",
    default = "sz",
    help = "Canton. Default: 'sz'")
  ...
  try:
    ...
    optparser.error("invalid overlap argument")
  settings.kanton=options.canton
...
def main():
  ...
  print my_settings.get_logfile_path()
